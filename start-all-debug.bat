...
chcp 65001 >nul
echo ═══════════════════════════════════════════════════════
echo    🚀 ResumeAI - Iniciando (Modo Debug)
echo ═══════════════════════════════════════════════════════
echo.
...
if %errorlevel% neq 0 (
    ...
    REM Iniciar Ollama con el modelo llama3.1 CON VENTANA VISIBLE
    start "🤖 Ollama - llama3.1 [DEBUG]" cmd /k "title 🤖 Ollama - llama3.1 [DEBUG] && color 0D && echo. && echo ═══════════════════════════════════════════════════════ && echo    🤖 OLLAMA - Modelo LLaMA 3.1 [DEBUG] && echo ═══════════════════════════════════════════════════════ && echo. && echo ✓ Puerto: 11434 && echo ✓ Modelo: llama3.1:latest && echo. && echo 📊 Logs en tiempo real: && echo ─────────────────────────────────────────────────────── && echo. && ollama run llama3.1"
...
start "🔧 Backend - FastAPI [DEBUG]" cmd /k "cd /d "%ROOT_DIR%\project" && title 🔧 Backend - FastAPI [DEBUG] && color 0A && venv\Scripts\activate.bat && echo. && echo ═══════════════════════════════════════════════════════ && echo    🚀 BACKEND - FastAPI con Hot Reload [DEBUG] && echo ═══════════════════════════════════════════════════════ && echo. && echo ✓ URL:  http://localhost:8000 && echo ✓ Docs: http://localhost:8000/docs && echo ✓ Hot Reload: ACTIVADO (--reload) && echo. && echo 📊 Logs en tiempo real: && echo ─────────────────────────────────────────────────────── && echo. && uvicorn app.main:app --reload --port 8000 --log-level info"

echo ✓ Backend iniciándose con hot reload...
echo   📍 URL: http://localhost:8000
echo   📚 Docs: http://localhost:8000/docs
echo   🔄 Hot Reload: ACTIVADO
echo   📊 Logs: Visibles en ventana "Backend - FastAPI [DEBUG]"
echo.
...
start "⚛️ Frontend - Next.js [DEBUG]" cmd /k "cd /d "%ROOT_DIR%\frontresume" && title ⚛️ Frontend - Next.js [DEBUG] && color 0B && echo. && echo ═══════════════════════════════════════════════════════ && echo    🌐 FRONTEND - Next.js con Fast Refresh [DEBUG] && echo ═══════════════════════════════════════════════════════ && echo. && echo ✓ URL: http://localhost:3000 && echo ✓ Fast Refresh: ACTIVADO (automático) && echo ✓ TypeScript: Check en tiempo real && echo. && echo 📊 Logs en tiempo real: && echo ─────────────────────────────────────────────────────── && echo. && npm run dev"

echo ✓ Frontend iniciándose con fast refresh...
echo   📍 URL: http://localhost:3000
echo   🔄 Fast Refresh: ACTIVADO
echo   📊 Logs: Visibles en ventana "Frontend - Next.js [DEBUG]"
echo.
...
echo.
echo 💡 TRES ventanas CON LOGS VISIBLES se han abierto:
echo    1. 🤖 Ollama - llama3.1 [DEBUG]     (verde oscuro) - Puerto 11434
echo    2. 🔧 Backend - FastAPI [DEBUG]     (verde)        - Puerto 8000
echo    3. ⚛️ Frontend - Next.js [DEBUG]    (azul)         - Puerto 3000
echo.
echo 🔄 HOT RELOAD ACTIVADO:
...
echo    • Interacciones con Ollama visibles en su ventana
echo.
...
echo.
echo 💡 Para modo normal (sin ventanas):
